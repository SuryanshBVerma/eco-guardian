...
const { GRADLE_BUILD_FILES } = require("../config/constants");
...
function normalizeGradleTask(task) {
  ...
  if (value === "dependencies" || value === ":dependencies") return "dependencies";
...
function projectDirFromTask(wrapperRoot, task) {
  ...
  return path.join(path.resolve(wrapperRoot), ...stripped.split(":").filter(Boolean));
}

function buildAutoTask(wrapperRoot, projectDir) {
  const relative = path.relative(path.resolve(wrapperRoot), path.resolve(projectDir));
...
function pickManifestPath(projectDir, foundFiles) {
  ...
  for (const candidate of GRADLE_BUILD_FILES) {
...
async function resolveGradleTaskPackages(roots, options, state) {
  ...
  if (explicitTask) {
    const wrapperRoot = await findGradleWrapperRoot(rootsArray[0] || process.cwd());
    if (!wrapperRoot) {
      return {
        packageMap,
        mode: "n/a",
    ...
    return {
      ...
      reason: packageMap.size > 0 ? "Gradle dependencies task" : "no resolved packages from task output",
