...
const { filterNestedNodeModules, dedupePaths, toRootPathWindows } = require('../shared/path-utils');
...
async function discoverWindowsDrives() {
  ...
  for (let i = 67; i <= 90; i += 1) {
    ...
    try { await fsp.access(root, fs.constants.R_OK); fallback.push(root); } catch (_) {}
  }
...
async function discoverNodeModulesViaNative(root) {
  if (PLATFORM === 'darwin') {
    const result = await runCommand('mdfind', ['-name', 'node_modules', '-onlyin', root], { timeoutMs: 30000 });
    if (!result.ok) return { ok: false, paths: [] };
    return { ok: true, paths: result.stdout.split(/\r?\n/).map((s) => s.trim()).filter(Boolean) };
  }
  if (PLATFORM === 'linux') {
    const result = await runCommand('locate', ['-b', '\\node_modules', '--existing'], { timeoutMs: 30000 });
    if (!result.ok) return { ok: false, paths: [] };
  ...
  if (PLATFORM === 'win32') {
    ...
    const result = await runCommand('cmd', ['/d', '/s', '/c', `dir /s /b /ad "${target}"`], { timeoutMs: 45000 });
    if (!result.ok) return { ok: false, paths: [] };
    return { ok: true, paths: result.stdout.split(/\r?\n/).map((s) => s.trim()).filter(Boolean) };
  }
...
async function discoverNodeModules(roots, options, counters) {
  ...
  const timer = setInterval(() => {
    process.stderr.write(`\r Searching for node_modules... ${counters.found} found`);
  }, 120);

  try {
    const nativePerRoot = await asyncPool(Math.min(API_CONCURRENCY, Math.max(1, roots.length)), roots, (root) => discoverNodeModulesViaNative(root));
    const nativeResults = [];
    for (const item of nativePerRoot) if (item && item.ok) nativeResults.push(...item.paths);
    ...
    log('success', `Found ${all.length} node_modules directories in ${hrSeconds(started)}s`, options);
    return all;
  } finally {
    clearInterval(timer);
    process.stderr.write('\r');
