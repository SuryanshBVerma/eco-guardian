...
function generateRemediationHint({ ecosystem, packageName, fixedVersion, foundIn }) {
  ...
  if (isDirect) {
    ...
    if (eco === 'nuget') return fixedVersion ? `Update ${packageName} to version ${fixedVersion} in project manifest.` : 'Review dependency usage.';
    return fixedVersion ? `Update ${packageName} to version ${fixedVersion}${scopeStr}.` : `Review ${packageName} usage${scopeStr}.`;
  }

  if (isTransitive) {
    const parentList = Array.from(parents);
    const parentStr = parentList.length > 0 ? ` via **${parentList.join(', ')}**` : '';
    const fixPart = fixedVersion ? ` A fix is available in version ${fixedVersion}.` : ' No known fix available yet.';
    
    if (eco === 'npm') return `Transitive dependency${parentStr}. Update the parent package(s) or use 'npm audit fix'.${fixPart}`;
