...
async function resolvePythonPackages(roots, options, state) {
  ...
  if (total > 1) {
    process.stderr.write(`[INFO] Resolving Python environments for ${total} directories...\n`);
  }

  const timer = total > 1 ? setInterval(() => {
    process.stderr.write(`\r Resolving Python environments... ${resolvedCount}/${total}`);
  }, 200) : null;

  try {
    await asyncPool(RESOLUTION_CONCURRENCY, rootsArray, async (root) => {
      try {
        const stdout = await require('./shared').execAsync('python -m pip inspect', { cwd: root });
        ...
        for (const dist of data.installed) {
          ...
          const pkg = createGraphPackage('python', name, version, [`${name}@${version}`], 1);
