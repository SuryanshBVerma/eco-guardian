...
async function resolveGoPackages(roots, options, state) {
  ...
  if (total > 1) {
    process.stderr.write(`[INFO] Resolving Go graphs for ${total} directories...\n`);
  }

  const timer = total > 1 ? setInterval(() => {
    process.stderr.write(`\r Resolving Go graphs... ${resolvedCount}/${total}`);
  }, 200) : null;

  try {
    await asyncPool(RESOLUTION_CONCURRENCY, rootsArray, async (root) => {
      try {
        const graphStdout = await require('./shared').execAsync('go mod graph', { cwd: root });
        const listStdout = await require('./shared').execAsync('go list -m -json all', { cwd: root });
        
        const versions = new Map();
        const modules = listStdout.split('\n}\n').filter(Boolean).map(s => JSON.parse(s + (s.endsWith('}') ? '' : '}')));
        for (const mod of modules) {
          if (mod.Version) versions.set(mod.Path, mod.Version);
        }
        
        const relations = graphStdout.split('\n').filter(Boolean).map(l => l.split(' '));
        ...
        const buildPath = (target, visited = new Set()) => {
          ...
          for (const [parent, child] of relations) {
            ...
            if (childPath === target) {
              ...
              return [...buildPath(pPath, visited), `${target}@${versions.get(target) || 'unknown'}`];
            }
          }
        ...
        for (const mod of modules) {
          ...
          const pkg = createGraphPackage('go', mod.Path, mod.Version, resolvedPath, resolvedPath.length);
          const key = pkg.key;
          
          const existing = packageMap.get(key);
          if (!existing) {
            pkg.paths.push(root);
            pkg.occurrences.push({ project: path.basename(root), manifest_path: path.join(root, 'go.mod'), dependency_type: resolvedPath.length === 1 ? 'direct' : 'transitive' });
