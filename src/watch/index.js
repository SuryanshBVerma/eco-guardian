...
const fs = require('fs')
const { loadWatchState, bootstrapState } = require('./state-store')
...
async function startWatchService (options, runtimeState = {}) {
  ...
  for (const input of snapshot.inputs) {
    ...
    try {
      const watcher = fs.watch(input.path, async (event, filename) => {
      ...
      watcher.on('error', (err) => {
        log('warn', `Watch error for ${input.path}: ${err.message}`, options)
      })
