...
async function resolveNpmPackages(roots, options, state) {
  ...
  if (total > 1) {
    process.stderr.write(`[INFO] Resolving npm graphs for ${total} directories...\n`);
  }

  const timer = total > 1 ? setInterval(() => {
    process.stderr.write(`\r Resolving npm graphs... ${resolvedCount}/${total}`);
  }, 200) : null;

  try {
    await asyncPool(RESOLUTION_CONCURRENCY, rootsArray, async (root) => {
      try {
        const stdout = await require('./shared').execAsync('npm ls --all --json', { cwd: root })
          .catch(err => {
        ...
        const walk = (dependencies, parentPath = [], depth = 0) => {
          ...
          for (const [name, info] of Object.entries(dependencies)) {
            ...
            const pkg = createGraphPackage('npm', name, version, currentPath, depth + 1);
            const key = pkg.key;
            ...
            if (!existing) {
              ...
              const alreadyHasParent = existing.occurrences.some(o => o.project === path.basename(root) && o.parent?.name === parent?.name);
              if (!alreadyHasParent) {
