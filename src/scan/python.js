    ...
    for (const section of sections) {
      ...
      for (const name of Object.keys(deps)) {
        ...
        if (info && info.version) {
          ...
          records.push(
            createPythonRecord(name, version, filePath, 'Pipfile.lock')
          )
        }
      }
    }
...
async function collectPythonPackages (roots, options, state) {
  ...
  log(
    'info',
    `Harvested ${totalEntries.toLocaleString()} Python dependency entries -> ${packageMap.size.toLocaleString()} unique combinations`,
    options
  )
  return packageMap
}
