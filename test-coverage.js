...
async function withTempDir(fn) {
  const root = await fsp.mkdtemp(path.join(os.tmpdir(), "eco-guardian-coverage-"));
  try {
...
async function testWhyAndVerboseLogging() {
  ...
  try {
    await withTempDir(async (root) => {
      ...
      const findings = [{ package: "axios", ecosystem: "npm", severity: "high", found_in: [] }];
...
async function testVScodeEcosystemScan() {
  ...
  try {
    await withTempDir(async (root) => {
      ...
      await fsp.writeFile(path.join(extDir, "package.json"), JSON.stringify({ name: "ext", version: "1.0.0", publisher: "pub" }));
...
async function testVulnerabilityCacheAndProviders() {
  ...
  pkgMap.set("npm|axios|1.0.0", { name: "axios", version: "1.0.0", ecosystem: "npm", paths: [] });
...
async function testFindingsBuilderAndFix() {
  const { buildFindings } = require("./src/findings/builder");
  const { pickBestFixedVersion, buildFixCommand, detectProjectOS } = require("./src/findings/fix");
  
  // 1. pickBestFixedVersion branches
  assert(pickBestFixedVersion([{ fixed_versions: ["  >= 1.2.3, < 2.0.0"] }]) === "1.2.3", "Clean version");
  assert(pickBestFixedVersion([{ fixed_versions: [null, ">= 1.2.3 < 2.0.0"] }]) === null, "Null/Range version skip");
  assert(pickBestFixedVersion([{ }]) === null, "Missing fixed_versions");

  // 2. buildFixCommand branches (ecosystems)
  assert(buildFixCommand({ ecosystem: "npm", isGlobal: true, fixedVersion: null }) === "npm uninstall -g undefined", "npm global uninstall");
  assert(buildFixCommand({ ecosystem: "Maven", packageName: "x", fixedVersion: "1" }).includes("mvn"), "Maven fix");
  assert(buildFixCommand({ ecosystem: "PyPI", packageName: "x", fixedVersion: "1" }).includes("pip"), "PyPI fix");
  assert(buildFixCommand({ ecosystem: "NuGet", packageName: "x", fixedVersion: "1" }).includes("dotnet"), "NuGet fix");
  assert(buildFixCommand({ ecosystem: "Go", packageName: "x", fixedVersion: "1" }).includes("go get"), "Go fix");
  
  // 3. buildFindings branches
  const pkgMap = new Map();
  pkgMap.set("k1", { name: "n1", version: "v1", ecosystem: "npm", paths: ["/p1"] });
  pkgMap.set("k2", { name: "n2", version: "v2", ecosystem: "maven", occurrences: [{ project: "/p2" }] });
  
  const vulnMap = {
    "k1": { vulnerable: true, advisories: [{ id: "a1", severity: "high", title: "t1", fixed_versions: ["1.0.1"] }] },
    "k2": { vulnerable: true, advisories: [{ id: "a2", severity: "low", title: "t2" }] },
  ...
  const manyStepsVulnMap = { "k": { vulnerable: true, advisories: [{ id: "a", severity: "high", title: "t", fixed_versions: ["1"] }] } };
  const manyFindings = await buildFindings(manyStepsPkgMap, manyStepsVulnMap, {});
  assert(manyFindings[0].fix_command.includes("+1 more"), "Should show +N more label");
}

async function testRemediationHints() {
  const { generateRemediationHint } = require("./src/findings/remediation");
  const ecosystems = ["npm", "maven", "nuget", "vscode", "python", "pypi", "go"];
  for (const eco of ecosystems) {
    const hint = generateRemediationHint({ ecosystem: eco, packageName: "pkg", fixedVersion: "1.0.0", foundIn: [{ project: "p", dependency_type: "direct" }] });
    ...
    const findings = [
      {
        ...
        advisory_id: "ID1",
        title: "Title & <script>",
        found_in: [{ project: "p1", dependency_type: "transitive", parent: { name: "parent" } }],
...
async function testNormalizers() {
  ...
  assert(n2.fixed_versions.length === 0, "No fixed versions for empty affected");

  // Weird severities
  const n3 = normalizeOsvAdvisory({ id: "3", database_specific: { severity: "MODERATE" } });
...
async function testDiscoveryDetails() {
  const { discoverScanRoots, discoverNodeModules } = require("./src/scan/discovery");
  ...
  try {
    ...
    await discoverScanRoots({ path: "C:\\", pathExplicit: false, global: true }, {});
    
    // 2. Native discovery via discoverNodeModules
    command.runCommand = async (cmd) => {
      if (cmd === "cmd") return { ok: true, stdout: "C:\\path\\node_modules\n" };
      return { ok: false };
    };
...
async function testNetworkErrors() {
  ...
  try {
    ...
    try { await providers.queryOsvForPackages([{ name: "x", version: "x" }], {}); } catch (_) {}
    ...
    https.request = (url, opts, cb) => {
      ...
      setTimeout(() => {
        cb(res);
        res.emit("data", JSON.stringify({ results: [{ vulns: [{ id: "V1" }] }] }));
        res.emit("end");
      }, 1);
      const req = new (require("events").EventEmitter)();
      req.setTimeout = () => {};
    ...
    await providers.queryOsvForPackages([{ name: "x", version: "v" }], { verbose: true });
...
async function testHarvestEdgeCases() {
  ...
  try {
    await withTempDir(async (root) => {
      ...
      assert(await readPackageJson(p1) === null, "Broken JSON should return null");
      ...
      assert(await readPackageJson(p2) === null, "Missing version should return null");
...
async function testCacheFailures() {
  ...
  try {
    ...
    fsp.mkdir = async () => { throw new Error("read only"); };
    ...
    fsp.readFile = async () => { throw new Error("read fail"); };
    fsp.writeFile = async () => { throw new Error("write fail"); };
    
    // Trigger via queryVulnerabilities with some packages
    const pkgMap = new Map();
    pkgMap.set("p", { key: "p", name: "n", version: "v", ecosystem: "npm" });
    try { await queryVulnerabilities(pkgMap, { noCache: false }); } catch (_) {}
  } finally {
    fsp.mkdir = originalMkdir;
...
async function testLocationEnrichment() {
  ...
  await withTempDir(async (root) => {
    ...
    await fsp.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "proj", dependencies: { "parent-pkg": "1.0.0" } }));
    
    // 1. Package-lock v2/v3 (packages field)
    const lockV2 = {
      packages: {
        "": { dependencies: { "parent-pkg": "1.0.0" } },
        "node_modules/parent-pkg": { version: "1.0.0", dependencies: { "target-pkg": "1.0.0" } }
      }
    };
    await fsp.writeFile(path.join(root, "package-lock.json"), JSON.stringify(lockV2));
    let locations = await enrichNpmLocations([pkgDir], "target-pkg", null);
    assert(locations[0].parent.name === "parent-pkg", "Should find parent in lock v2");

    // 2. Package-lock v1 (dependencies field)
    const lockV1 = {
      dependencies: {
        "parent-pkg-v1": { version: "1.0.0", requires: { "target-pkg": "1.0.0" } }
      }
    };
    await fsp.writeFile(path.join(root, "package-lock.json"), JSON.stringify(lockV1));
    locations = await enrichNpmLocations([pkgDir], "target-pkg", null);
    assert(locations[0].parent.name === "parent-pkg-v1", "Should find parent in lock v1");

    // 3. Yarn lock
    await fsp.rm(path.join(root, "package-lock.json"));
    const yarnLock = 'parent-yarn@^1:\n  version "1.0.0"\n  dependencies:\n    target-pkg "1.0.0"\n';
    await fsp.writeFile(path.join(root, "yarn.lock"), yarnLock);
    locations = await enrichNpmLocations([pkgDir], "target-pkg", null);
    assert(locations[0].parent.name === "parent-yarn", "Should find parent in yarn lock");
  });
}
...
async function testDiscoveryNativeMocks() {
  ...
  try {
    for (const plat of ["darwin", "linux"]) {
      ...
      command.runCommand = async () => ({ ok: true, stdout: "/mock/node_modules\n" });
...
async function testRunScanOrchestrationDetails() {
  ...
  try {
    await withTempDir(async (root) => {
      ...
      await runScan({ path: path.join(root, "non-existent"), pathExplicit: true, ecosystems: ["npm"], json: true });
