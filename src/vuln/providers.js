...
async function queryOsvForPackages(packages, options) {
  ...
  log('info', `Querying OSV database... (${chunks.length} batches, ${packages.length.toLocaleString()} packages)`, options);
  ...
  if (uniqueVids.size > 0) {
    await asyncPool(API_CONCURRENCY, Array.from(uniqueVids), async (vid) => {
      try {
        ...
        log('error', `Failed to fetch full details for ${vid}: ${err.message}`, options);
      }
    });
  }
