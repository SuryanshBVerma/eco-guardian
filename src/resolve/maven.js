...
async function resolveMavenPackages(roots, options, state) {
  ...
  if (total > 1) {
    process.stderr.write(`[INFO] Resolving Maven graphs for ${total} directories...\n`);
  }

  const timer = total > 1 ? setInterval(() => {
    process.stderr.write(`\r Resolving Maven graphs... ${resolvedCount}/${total}`);
  }, 200) : null;

  try {
    await asyncPool(RESOLUTION_CONCURRENCY, rootsArray, async (root) => {
      try {
        const stdout = await require('./shared').execAsync('mvn dependency:tree -DoutputType=text', { cwd: root });
        ...
        for (const line of lines) {
          ...
          const currentPath = [...stack.map(s => `${s.name}@${s.version}`), `${name}@${version}`];
          stack.push({ name, version });
          
          const pkg = createGraphPackage('maven', name, version, currentPath, depth);
          const key = pkg.key;
