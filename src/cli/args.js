...
function parseArgs (argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (token === '--export-sarif') {
      const next = argv[i + 1]
      if (!next || next.startsWith('--')) { throw new Error('Missing value for --export-sarif') }
    ...
    if (token === '--baseline') {
      const next = argv[i + 1]
      if (!next || next.startsWith('--')) { throw new Error('Missing value for --baseline') }
      args.baseline = next
      i += 1
      continue
    }
    if (token === '--write-baseline') {
      const next = argv[i + 1]
      if (!next || next.startsWith('--')) { throw new Error('Missing value for --write-baseline') }
    ...
    if (token === '--why') {
      const next = argv[i + 1]
      if (!next || next.startsWith('--')) { throw new Error('Missing value for --why') }
      args.why = next
      i += 1
      continue
    }
