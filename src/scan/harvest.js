...
async function readPackageJson(packageDir) {
  ...
  try { raw = await fsp.readFile(file, 'utf8'); } catch (error) {
    if (error && (error.code === 'ENOENT' || error.code === 'ENOTDIR' || error.code === 'EACCES' || error.code === 'EPERM')) return null;
    return null;
  }

  let parsed;
  try { parsed = JSON.parse(raw); } catch (_) { return null; }
  if (!parsed || typeof parsed.name !== 'string' || typeof parsed.version !== 'string' || !parsed.name || !parsed.version) return null;
  return { name: parsed.name, version: parsed.version };
}

async function listPackagesUnderNodeModules(nodeModulesDir) {
  let entries;
  try { entries = await fsp.readdir(nodeModulesDir, { withFileTypes: true }); } catch (_) { return []; }
  ...
  for (const entry of entries) {
    ...
    if (entry.name.startsWith('@')) {
      let scoped;
      try { scoped = await fsp.readdir(path.join(nodeModulesDir, entry.name), { withFileTypes: true }); } catch (_) { continue; }
      for (const scopedEntry of scoped) {
        ...
        packageDirs.push(path.join(nodeModulesDir, entry.name, scopedEntry.name));
      }
      continue;
    }
...
async function harvestPackages(nodeModulesDirs, options, state) {
  ...
  log('info', `Harvested ${totalEntries.toLocaleString()} package entries -> ${packageMap.size.toLocaleString()} unique name@version pairs`, options);
  if (packageMap.size > 50000) log('warn', `Large scan detected (${packageMap.size.toLocaleString()} unique packages). This may take longer.`, options);
