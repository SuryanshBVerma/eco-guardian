...
function normalizeState(input = {}) {
  const state = { ...input };
  state.path = state.path == null ? os.homedir() : String(state.path);
  state.pathExplicit = Boolean(state.pathExplicit);
  ...
  state.exportSarif = state.exportSarif == null ? null : String(state.exportSarif);
  ...
  state.maxHigh =
    state.maxHigh == null || state.maxHigh === "" ? null : Number(state.maxHigh);
  ...
  state.alertsFile = state.alertsFile == null ? DEFAULT_ALERTS_FILE : String(state.alertsFile);
...
function buildCommand(inputState = {}, options = {}) {
  ...
  if (state.pathExplicit && !state.globalOnly && state.path) {
    pushFlag(parts, "--path", state.path, platform);
  }
  if (state.globalOnly) parts.push("--global-only");
  if (state.ecosystems.length > 0 && !(state.ecosystems.length === 1 && state.ecosystems[0] === "npm")) {
  ...
  if (state.gradleTask && state.graphResolution && state.ecosystems.includes("gradle")) {
    pushFlag(parts, "--gradle-task", state.gradleTask, platform);
  }
  if (state.dependencyCheckMode && state.nvdMode === "on") {
  ...
  if (state.nvdMode !== "off" && state.nvdApiKey && state.ecosystems.some((item) => item === "maven" || item === "gradle")) {
  ...
  if (state.maxCritical != null) pushFlag(parts, "--max-critical", state.maxCritical, platform);
  if (state.maxHigh != null) pushFlag(parts, "--max-high", state.maxHigh, platform);
  pushFlag(parts, "--why", state.why, platform);
  if (state.benchmark) parts.push("--benchmark");
  if (state.watch) {
    parts.push("--watch");
    if (state.notifyOnSeverity && state.notifyOnSeverity !== DEFAULT_NOTIFY_SEVERITY) {
    ...
    if (state.reconcileInterval !== DEFAULT_RECONCILE_INTERVAL_SEC) {
      pushFlag(parts, "--reconcile-interval", state.reconcileInterval, platform);
    }
