...
async function collectMavenPackages(roots, options, state) {
  ...
  const manifestDirs = await discoverManifestFiles(roots, MAVEN_MANIFEST_NAMES, options, counters, 'Maven POMs');
  ...
  log('info', `Harvested ${totalEntries.toLocaleString()} Maven dependency entries -> ${packageMap.size.toLocaleString()} unique combinations`, options);
  return packageMap;
}
