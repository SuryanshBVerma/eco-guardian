...
function parsePackagesConfig(xmlText, filePath) {
  ...
  for (const pkg of pkgElems) {
    ...
    if (id && version) records.push(createNuGetRecord(id, version, filePath, 'packages.config'));
  }
  return records;
}

function parseProjectPackageReferences(xmlText, filePath, centralVersions = new Map()) {
  const records = [];
  ...
  for (const ref of refElems) {
    ...
    if (id && version) {
      records.push(createNuGetRecord(id, version, filePath, path.basename(filePath)));
    }
  }
  return records;
}
...
async function collectNuGetPackages(roots, options, state) {
  ...
  const manifestDirs = await discoverManifestFiles(roots, NUGET_MANIFEST_NAMES, options, counters, 'NuGet manifests');
  ...
  log('info', `Harvested ${totalEntries.toLocaleString()} NuGet dependency entries -> ${packageMap.size.toLocaleString()} unique combinations`, options);
  return packageMap;
}
