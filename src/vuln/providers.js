      ...
      (res) => {
        ...
        res.on("end", () => {
          if (res.statusCode === 429) {
            ...
            err.retryAfterMs = parseInt(res.headers["retry-after"] || "30", 10) * 1000;
            return reject(err);
          }
...
function makeNvdThrottle(hasApiKey) {
  const INTERVAL_MS = hasApiKey ? NVD_REQUEST_INTERVAL_MS_AUTH : NVD_REQUEST_INTERVAL_MS_UNAUTH;
  let lastFireTime = 0;
  return {
    async acquire() {
      const now = Date.now();
...
async function queryNvdByCpe(artifactId, version, groupId = null, options, diagnostics = null, throttle = null, label = "") {
  const cleanV = String(version || "").split(":")[0].replace(/^v/, "");
  ...
  for (const product of productNames) {
    ...
    let cves = await _nvdFetch(url, headers, artifactId, options, diagnostics, throttle);
    ...
    if (cves.length === 0 && groupId) {
      ...
      cves = await _nvdFetch(url, headers, artifactId, options, diagnostics, throttle);
    }
  ...
  if (allCves.length === 0) {
    ...
    const cves = await _nvdFetch(url, headers, artifactId, options, diagnostics, throttle);
    for (const cve of cves) {
      if (cve && cve.id && !seenIds.has(cve.id)) {
 ...
 */
async function _nvdFetch(url, headers, artifactId, options, diagnostics, throttle) {
  let attempt = 0;
  while (attempt < 3) {
    if (throttle) await throttle.acquire();
    try {
      const data = await httpsGet(url, 15000, headers);
      const items = data && Array.isArray(data.vulnerabilities) ? data.vulnerabilities : [];
      ...
      if (err.message && err.message.includes("429")) {
        ...
        log("warn", `NVD rate-limited, waiting ${Math.ceil(waitMs / 1000)}s before retry...`, options);
        await delay(waitMs);
        attempt += 1;
        continue;
      }
      if (diagnostics) diagnostics.nvdErrors = (diagnostics.nvdErrors || 0) + 1;
      log("warn", `NVD query failed for ${artifactId}: ${err.message}`, options);
      return [];
    }
  }

  if (diagnostics) diagnostics.nvdErrors = (diagnostics.nvdErrors || 0) + 1;
  log("warn", `NVD query failed for ${artifactId} after ${attempt} retries`, options);
  return [];
}
