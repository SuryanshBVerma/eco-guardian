...
async function testNpmResolver() {
  const jsonContent = JSON.stringify({
    dependencies: {
      'axios': { version: '1.0.0', dependencies: { 'follow-redirects': { version: '1.15.0' } } }
    }
  });
  ...
  assert(map.has('npm|follow-redirects|1.15.0'), 'npm resolver should find transitives');
  assert(map.get('npm|follow-redirects|1.15.0').depth === 2, 'transitive depth should be 2');
  assert(map.get('npm|follow-redirects|1.15.0').resolved_path.includes('axios@1.0.0'), 'path should include parent');
}
...
[INFO] \\- com.google.guava:guava:jar:33.0.0-jre:compile
  ...
  assert(map.has('maven|org.slf4j:slf4j-api|1.7.36'), 'maven resolver should find slf4j-api');
  assert(map.get('maven|org.slf4j:slf4j-api|1.7.36').depth === 1, 'slf4j-api depth should be 1');
  assert(map.get('maven|org.slf4j:slf4j-parent|1.7.36').depth === 2, 'slf4j-parent depth should be 2');
}

async function testNuGetResolver() {
  require('./src/resolve/shared').execAsync = async () => JSON.stringify({
    projects: [{
      path: 'C:\\proj\\test.csproj',
      frameworks: [{
        topLevelPackages: [{ id: 'Newtonsoft.Json', resolvedVersion: '13.0.3' }],
        transitivePackages: [{ id: 'Some.Transitive', resolvedVersion: '1.0.0' }]
      }]
    }]
  });

  const map = await resolveNuGetPackages(['/root'], { verbose: false }, {});
  assert(map.has('nuget|Newtonsoft.Json|13.0.3'), 'nuget resolver should find Newtonsoft.Json');
  assert(map.get('nuget|Some.Transitive|1.0.0').resolution_mode === 'graph', 'nuget should be in graph mode');
}
...
async function testGoResolver() {
  ...
  require('./src/resolve/shared').execAsync = async (cmd) => {
    ...
    return JSON.stringify({ Path: 'my-app', Main: true }) + '\n}\n' + 
           JSON.stringify({ Path: 'github.com/gin-gonic/gin', Version: 'v1.9.1' }) + '\n}\n' +
           JSON.stringify({ Path: 'github.com/go-playground/validator/v10', Version: 'v10.14.0' });
  };

  const map = await resolveGoPackages(['/root'], { verbose: false }, {});
  assert(map.has('go|github.com/gin-gonic/gin|v1.9.1'), 'go resolver should find gin');
  assert(map.has('go|github.com/go-playground/validator/v10|v10.14.0'), 'go resolver should find transitives');
  assert(map.get('go|github.com/go-playground/validator/v10|v10.14.0').depth === 2, 'go transitive depth check');
}
...
async function testPythonResolver() {
  ...
  assert(map.has('python|requests|2.31.0'), 'python resolver should find requests');
